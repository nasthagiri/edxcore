...
from .. import get_request_or_stub
from ..middleware import RequestCache, request_cached
...
class TestRequestCache(TestCase):
    ...
    def _dummy_task(self):
        ...
        cache = {"test_cache": "blah blah blah"}
        RequestCache.get_request_cache().data.update(cache)

    def test_clear_cache_celery(self):
        """ Test that the request cache is cleared after a task is run. """
        self._dummy_task.apply(args=(self,)).get()  # pylint: disable=no-member
        self.assertEqual(RequestCache.get_request_cache().data, {})
